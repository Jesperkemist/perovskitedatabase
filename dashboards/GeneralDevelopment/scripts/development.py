# =============================================================================
# Bokeh app focusing on general development (HTL, ETL, deposition tecniques, etc.)
# 
# By Jesper Jacobsson
# 2020 06
# =============================================================================
...
def make_plot(source, data, view, alphaValue, booleanCategory, colorLabels, currentRowsInData, legendCategory, markerSize, newPlotWidth, tooltips, useColorMarkers, xAxisLogStart, xrange, x_axis, x_scale_select, yAxisLogStart, y_axis, y_scale_select):
    ''' Generat the plot'''
    ...
    if x_axis[1] == 'Ref_publication_date': 
        ...
        #p.x_range=Range1d(10**xAxisLogStart, 1.02*max(source.data[x_axis[1]]))
        p.x_range=Range1d(10**xAxisLogStart, 1.02*max(x_selected[x_axis[1]]))
    else:
        #p.x_range=Range1d(0, 1.02*max(source.data[x_axis[1]]))
        p.x_range=Range1d(0, 1.02*max(x_selected[x_axis[1]]))
...
def tableColumnsForSelectedData():
    ...
    tableColumns = [
        ...
        TableColumn(field='Cell_area_measured', title='Cell Area [cm^2]', width = 60),
        TableColumn(field='ETL_stack_sequence', title='Electron transport layer', width = 1000),
        TableColumn(field='HTL_stack_sequence', title='Hole transport layer', width = 1000),
...
def interactiveEngine():
    ...
    def select_data(data):
        ...
        # Remove rows with nan values in the categories that should be plotted (nan values have previously been set to -1)
    ...
    def updatePlot():
        ...
        # Initiate the figure
        p, GlyphRenderer = make_plot(source, mainDataFrame,
                  view = view,
