...
def make_plot(source, data, view, alphaValue, booleanCategory, colorLabels, currentRowsInData, legendCategory, legendFontSize, markerSize, newPlotWidth, tooltips, useColorMarkers, xAxisLogStart, xrange, x_axis, x_scale_select, yAxisLogStart, y_axis, y_scale_select):
    ''' Generat the plot'''
...
def tableColumnsForSelectedData():
    ...
    tableColumns = [
        ...
        TableColumn(field='Module_area_total', title='Area [cm^2]', width = 60),
        TableColumn(field='Module_area_effective', title='Area [cm^2]', width = 60),
        TableColumn(field='Module_number_of_cells_in_module', title='Number of cells in module', width = 60),
        TableColumn(field='ETL_stack_sequence', title='Electron transport layer', width = 1000),
...
def interactiveEngine():
    ...
    def updateMainDataFrame(newCategories):
        ...
        # Add the ID column and remove dublets
            ...
            # Read in new data
            newData = loadData_withBoleanFilter(dataColumns = newCategories, boleanColumn = 'Module', engine = engine)
    ...
    mainDataFrame = loadData_withBoleanFilter(dataColumns = dataColumnsToUseFromTheStart(), boleanColumn = 'Module', engine = engine)
    ...
    for i, item in enumerate(multiselectCategories):
        multiselectDict[item] = databaseCatagoriesUnique_withBoleanFilter(column = item, boleanColumn = 'Module', engine = engine)
    ...
    for i, item in enumerate(multiselectCategories):
        multiselectDictShort[item] = databaseCategoriesMostCommon_withBoleanFilter(column = item, boleanColumn = 'Module', number = numberOfMostComonAlternatives, engine = engine)
    ...
    layout_tab1 = row(controls1, p, controls2, controls3, controls4, controls5)
