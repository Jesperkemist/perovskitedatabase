# =============================================================================
# Bokeh app focusing on outdoor testing
# 
# By Jesper Jacobsson
# 2020 08
# =============================================================================
...
def make_plot(source, data, view, alphaValue, booleanCategory, colorLabels, currentRowsInData, legendCategory, legendFontSize, markerSize, newPlotWidth, tooltips, useColorMarkers, xAxisLogStart, xrange, x_axis, x_scale_select, y_axis, y_scale_select):
    ''' Generat the plot'''
...
def tableColumnsForSelectedData():
    ...
    tableColumns = [
        ...
        TableColumn(field='Outdoor_PCE_initial_value', title='Initial PCE [%]', formatter=NumberFormatter(format="0.00"), width = 100),
        TableColumn(field='Outdoor_time_total_exposure', title='Measurement time [h]', formatter=NumberFormatter(format="0.00"), width = 130),
        TableColumn(field='Outdoor_PCE_after_1000_h', title='PCE after 1000 h [%]', formatter=NumberFormatter(format="0.00"), width = 130),
        TableColumn(field='Outdoor_PCE_end_of_experiment', title='PCE_f/PCE_i [%]', formatter=NumberFormatter(format="0.00"), width = 130),
        TableColumn(field='Outdoor_PCE_T80', title='T80 [h]', formatter=NumberFormatter(format="0.00"), width = 50),
        TableColumn(field='Outdoor_PCE_Ts80', title='Ts80 [h]', formatter=NumberFormatter(format="0.00"), width = 50),
        TableColumn(field='Outdoor_PCE_Te80', title='Te80 [h]', formatter=NumberFormatter(format="0.00"), width = 50),
        TableColumn(field='Outdoor_PCE_Tse80', title='Tse80 [h]', formatter=NumberFormatter(format="0.00"), width = 55),
        TableColumn(field='Outdoor_PCE_T95', title='T95 [h]', formatter=NumberFormatter(format="0.00"), width = 50),
        TableColumn(field='Outdoor_PCE_Ts95', title='Ts95 [h]', formatter=NumberFormatter(format="0.00"), width = 50),
...
def interactiveEngine():
    ...
    def select_data(data):
        ...
        # The data range slider apears to behae diferently on diferent systems
    ...
    def updateMainDataFrame(newCategories):
        ...
        # Add the ID column and remove dublets
            ...
            # Read in new data
            newData = loadData_withBoleanFilter(dataColumns = newCategories, boleanColumn = 'Outdoor_tested', engine = engine)
    ...
    mainDataFrame = loadData_withBoleanFilter(dataColumns = dataColumnsToUseFromTheStart(), boleanColumn = 'Outdoor_tested', engine = engine)
    ...
    for i, item in enumerate(multiselectCategories):
        multiselectDict[item] = databaseCatagoriesUnique_withBoleanFilter(column = item, boleanColumn = 'Outdoor_tested', engine = engine)
    ...
    for i, item in enumerate(multiselectCategories):
        multiselectDictShort[item] = databaseCategoriesMostCommon_withBoleanFilter(column = item, boleanColumn = 'Outdoor_tested', number = numberOfMostComonAlternatives, engine = engine)
    ...
    legendCategory_map = {
        ...
        'City' : 'Outdoor_location_city',
        'Climate zone' : 'Outdoor_location_climate_zone',
        'Country' : 'Outdoor_location_country',
        ...
        'Measurement protocoll' : 'Outdoor_protocol',
        'Number of tracking axis' : 'Outdoor_installation_number_of_solar_tracking_axis',
        ...
        'Potential load condition' : 'Outdoor_potential_bias_load_condition',
        'Season' : 'Outdoor_time_season',
        'Substrate': 'Substrate_stack_sequence',
        'Temperature load condition' : 'Outdoor_temperature_load_condition',
        }
    x_axis_map = {
        'Cell area [cm^2]' : 'Cell_area_measured',
        'Initial PCE [%]' : 'Outdoor_PCE_initial_value',
        'Measurement time [days]' : 'Outdoor_time_total_exposure',
        'Power generated [kWh/year/m^2]' : 'Outdoor_power_generated',
    ...
    y_axis_map = {
        "PCE after 1000 h [%]": "Outdoor_PCE_after_1000_h",
        "PCE_f/PCE_i [%]" : "Outdoor_PCE_end_of_experiment",
        "T80 [h]": "Outdoor_PCE_T80",
        "Ts80 [h]": "Outdoor_PCE_Ts80",
        "Te80 [h]": "Outdoor_PCE_Te80",
        "Tse80 [h]": "Outdoor_PCE_Tse80",
        "T95 [h]": "Outdoor_PCE_T95",
        "Ts95 [h]": "Outdoor_PCE_Ts95",
        'Power generated [kWh/year/m^2]' : 'Outdoor_power_generated',
        }
    y_scale_select_map = {
        "Linear": 'linear',
    ...
    selects = {
        ...
        'x_axis' : Select(title="X-Axis", options=list(x_axis_map.keys()), value="Measurement time [days]"),
        'x_scale_select' : Select(title="X-Axis scale", options=list(x_scale_select_map.keys()), value="Linear"),
        'y_axis' : Select(title="Y-Axis", options=list(y_axis_map.keys()), value="PCE_f/PCE_i [%]"),
